...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
	...
	new Thread(new Runnable() {
	  ...
	  public void run() {
		...
		try {
		  ...
		  Log.d("priyam", "run: using firebase ");
		...
		runOnUiThread(new Runnable() {
		  ...
		  public void run() {
			...
			btnSign.setFocusable(true);
			  ...
			  Log.d("PriyamVersion", "run: signin");
			...
			btnSign.setOnClickListener(new View.OnClickListener() {
			  ...
			  public void onClick(View v) {
				...
				// Create and launch sign-in intent
				  Log.d("PriyamVersion", "run: signin-started");
				startActivityForResult(
				  ...
				  Log.d("PriyamVersion", "run: signin done");
			  }
			});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Log.d("PriyamVersion", "on Activity Result" + requestCode + " "+RC_SIGN_IN + " "+resultCode+ " "+RESULT_OK);
		if ((requestCode == RC_SIGN_IN && resultCode == RESULT_OK)) {
			// Successfully signed in
			Log.d("PriyamVersion", "startingUp");
			startApp();
		}
	}
  ...
  private void startApp() {
	  ...
	  Log.d("PriyamVersion", "startApp: Sign in done");

	  FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();

	if (user == null) {

		Log.d("PriyamVersion", "it's NULL");
	  Toast.makeText(this, "Login error", Toast.LENGTH_SHORT).show();
	  return;
	}

	  Log.d("PriyamVersion", "NOT NULL");
